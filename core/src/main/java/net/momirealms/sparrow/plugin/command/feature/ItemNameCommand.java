...
public final class ItemNameCommand extends BukkitCommandFeature {
    public ItemNameCommand(@NotNull CommandManager commandManager, @NotNull SparrowPlugin plugin) {
        ...
        this.plugin().scheduler().platform().run(() -> {
            ...
            if (input != null) {
                ...
                try {
                    ...
                    this.handleFeedback(context, MessageConstants.COMMAND_ITEM_NAME_INVALID);
                    return;
                }
                item.set(DataComponents.ITEM_NAME, minecraftName);
                this.handleFeedback(context, MessageConstants.COMMAND_ITEM_NAME_SUCCESS, name, Component.text(player.getName()));
                return;
            }
            net.minecraft.network.chat.Component name = item.get(DataComponents.ITEM_NAME);
            if (name == null) {
                this.handleFeedback(context, MessageConstants.COMMAND_ITEM_NAME_UNNAMED, Component.text(player.getName()));
                return;
            }
            ...
            Component editHint = MessageConstants.COMMAND_ITEM_NAME_EDIT.build();
            ...
            this.handleFeedback(context, MessageConstants.COMMAND_ITEM_NAME_QUERY, preview, jsonEditor, miniMessageEditor, Component.text(player.getName()));
        }, () -> {}, player);
    }
